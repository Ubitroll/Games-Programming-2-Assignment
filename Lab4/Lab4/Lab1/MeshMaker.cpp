...
void Mesh::init(Vertex* vertices, unsigned int numVertices, unsigned int* indices, unsigned int numIndices)
{
	...
	{
		model.positions.push_back(*vertices[i].FindPosition());
		model.textureCoordinates.push_back(*vertices[i].FindTextrueCoordinates());
...
void Mesh::initialiseModel(const IndexedModel& model)
{
	...
	glBufferData(GL_ARRAY_BUFFER, model.positions.size() * sizeof(model.textureCoordinates[0]), &model.textureCoordinates[0], GL_STATIC_DRAW); //move the data to the GPU - type of data, size of data, starting address (pointer) of data, where do we store the data on the GPU
